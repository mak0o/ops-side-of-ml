...
初めての自動入れ替えは、300 行のうち 3 行の違いで決まっていた。差が偶然でないかを確かめる条件
（判定が食い違った行の片側 McNemar 検定、α = 0.05）を加え、今はこの規模の差では入れ替わらない
（[差が偶然でないかを確かめる](#差が偶然でないかを確かめる)）。その過程で、ブートストラップが
もっともらしい数値を出しながら実は何も測っていなかったことも分かった。
