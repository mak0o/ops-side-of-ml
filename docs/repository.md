...
### 使わない期間は GitHub からの経路を切る

しばらく触らない間は、`enable_github_oidc = false` で OIDC プロバイダを削除している。
3 つのロールは残るが、信頼先のプロバイダが存在しないので誰も引き受けられない。
GitHub アカウントが乗っ取られても AWS には届かない。

信頼ポリシーではプロバイダのリソースを参照せず、ARN を文字列で組み立てている。
リソースを参照すると、プロバイダを消したときにロールまで作り直しになるため。

切っている間は CI の plan / apply / image が認証で失敗する（想定どおり）。
戻すときは、先に手元から `-var enable_github_oidc=true` で apply してから、tfvars の変更をマージする。
CI から戻すことはできない。

再開するときは、プロバイダを戻したあとに次を確認する。
切っている間に `src/` を変更していれば ECR に新しいイメージが無いので、image ワークフローを手動で実行する。
推論ログ（90 日）や ECR のタグなしイメージ（1 日）はライフサイクルで消えているので、
パイプラインを動かすなら `scripts/prepare_datasets.py` でデータを置き直す。
...
| Dependabot | アクションの更新を週 1 回提案させる。マイナー・パッチ版は 1 つの PR にまとめ、メジャー版はアクションごとの PR に分ける |
